...
CREATE TABLE recording_schedules (
    ...
    detection_areas JSON,
    post_motion_duration_seconds INT,
...
CREATE TABLE recording_schedule_time_slots (
    ...
    PRIMARY KEY (schedule_id, day_of_week, start_time)
);
...
CREATE TABLE motion_events (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_motion_events_camera (camera_id),
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/cameras/statistics/summary` | GET | 摄像头统计聚合 |
| `/api/recordings/{id}/restore` | POST | 恢复已删除录像 |
| `/api/recordings/orphaned` | GET | 查询孤立录像 |
| `/api/recordings/cleanup` | POST | 手动触发清理 |
| `/api/recordings/schedules` | GET/POST | 录像计划CRUD |
| `/api/recordings/schedules/{id}` | GET/PUT/DELETE | 录像计划详情 |
| `/api/recordings/schedules/{id}/toggle` | PATCH | 启用/禁用 |
| `/api/recordings/motion-events` | GET | 移动侦测历史 |
| `/api/edge/recordings/schedules` | GET | 节点同步录像计划 |
| `/api/edge/recordings/motion-events` | POST | 节点上报移动事件 |
